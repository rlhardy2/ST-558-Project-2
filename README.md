...
-   <a href="#writing-the-functions" id="toc-writing-the-functions">Writing
    ...
    -   <a href="#function-number-three-listbydistance"
        ...
        listByDistance()</a>
    -   <a href="#function-number-four" id="toc-function-number-four">Function
        Number Four:</a>
...
be using an API that is focused on breweries. Each function will return
a data frame with variables such as brewery ID, name, type, address
(street, city, and state variables), postal code, country, longitude,
latitude, phone number, and website URL to name a few. Each function
...
listByState <- function(state = "north_carolina", length = 20) {
  ...
  #Make sure state is all lowercase with underscorse instead of spaces.
  state <- tolower(state)
  state <- sub(" ", "_", state)
...
The fourth function is a function…
